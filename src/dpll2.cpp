...
bool sign(int literal) {
    return literal > 0;
}


result bcp(vector<vector<int>>& formula, set<int>& assignment) {
    bool changed = true;
    while (changed) {
        changed = false;
...
bool hasUnitClause(vector<vector<int>> formula){
...
result plp(vector<vector<int>>& formula, set<int>& assignment) {
    set<int> seenPositive;
    set<int> seenNegative;
...
//        assignment.insert(literal);
        ...
        formula.erase(
                remove_if(formula.begin(), formula.end(), [&](const auto& clause) {
...
bool dpll(vector<vector<int>>& formula1, set<int> assignment1) {
    result re;
    result result;
    ...
    vector<vector<int>> formula = result.CNF;
    set<int> assignment = result.Assignment;
    ...
    set<int> assignment_copy = assignment;
